...
Проверка охватывает:
- наличие и структуру YAML-фронтматтера,
...
Утилита рекурсивно обходит каталоги и проверяет все подходящие файлы.
...
- 🔁 **Автоматическая проверка при сохранении**:  
  Работает для всех `.md` и `.ct` файлов. Можно отключить в настройках:
  ```json
  {
